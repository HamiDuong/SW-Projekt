...
class EventMapper(Mapper):
    ...
    def find_all(self):
        ...
        for (
            ...
            event.set_break_begin_id(break_begin_id), event.set_break_end_id(
                break_end_id
            ),
            event.set_illness_begin_id(illness_begin_id), event.set_illness_end_id(
                illness_end_id
            ),
            event.set_project_work_begin_id(
                project_work_begin_id
            ), event.set_project_work_end_id(project_work_end_id),
            event.set_vacation_begin_id(vacation_begin_id), event.set_vacation_end_id(
                vacation_end_id
            ),
            event.set_flex_day_start_id(flex_day_start_id), event.set_flex_day_end_id(
                flex_day_end_id
            )
            result.append(event)
    ...
    def find_by_key(self, key):
        ...
        command = "SELECT id, date_of_last_change, type, coming_id, going_id, break_begin_id, break_end_id,\
                    illness_begin_id, illness_end_id, project_work_begin_id, project_work_end_id, \
                    vacation_begin_id, vacation_end_id, flex_day_start_id, flex_day_end_id FROM worktimeapp.event WHERE id={}".format(
            key
        )
        cursor.execute(command)
        tuples = cursor.fetchall()
    ...
    def find_by_foreign_key_and_type(self, foreign_key, key, type):
        ...
        command = "SELECT id, date_of_last_change, type, coming_id, going_id, break_begin_id, break_end_id,\
                    illness_begin_id, illness_end_id, project_work_begin_id, project_work_end_id, \
                    vacation_begin_id, vacation_end_id, flex_day_start_id, flex_day_end_id FROM worktimeapp.event WHERE {}={} AND type='{}'".format(
    ...
    def update(self, event):
        ...
        command = "UPDATE worktimeapp.event " + "SET date_of_last_change=%s WHERE id=%s"
        data = (event.get_date_of_last_change(), event.get_id())
    ...
    def delete(self, event):
        ...
        command = "DELETE FROM worktimeapp.event WHERE id={}".format(event.get_id())
        cursor.execute(command)
