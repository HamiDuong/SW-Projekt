...
class ActivityMapper(Mapper):
    ...
    def find_by_key(self, key):
        ...
        if tuples[0] is not None:
            (id, dateOfLastChange, name, capacity, projectId) = tuples[0]
            activityobj = ActivityBO()
            activityobj.set_id(id)
            activityobj.set_date_of_last_change(dateOfLastChange)
            activityobj.set_name(name)
    ...
    def insert (self, activity_obj):
        ...
        command = "INSERT INTO activities (id, dateOfLastChange, name, capacity, projectId) VALUES (%s, %s, %s, %s, %s)"
    ...
    def update (self, activity_obj):
        ...
        command = "UPDATE activities " + "SET name=%s, capacity=%s, dateOfLastChange=%s WHERE id=%s"
        data = (activity_obj.get_name(), activity_obj.get_capacity(), activity_obj.get_date_of_last_change() ,activity_obj.get_id())
        cursor.execute(command, data)
    ...
    def find_all_by_project_id(self, project_id):
        result = []
        cursor = self._cnx.cursor()
        cursor.execute("SELECT * from activities WHERE projectId={}".format(project_id))
        tuples = cursor.fetchall()
