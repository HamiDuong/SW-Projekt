...
class IllnessMapper(TimeIntervalMapper):
    ...
    def find_all(self):
        ...
        cursor.execute(
            "SELECT id, dateOfLastChange, start, end, startEvent, endEvent, type from worktimeapp.illnesses")
    ...
    def find_by_key(self, key):
        result = None
        cursor = self._cnx.cursor()
        command = "SELECT id, dateOfLastChange, start, end, startEvent, endEvent, type from worktimeapp.illnesses WHERE id={}".format(
            key)
    ...
    def insert(self, illness):
        ...
        data = (illness.get_id(), illness.get_date_of_last_change(), illness.get_start(
        ), illness. get_end(), illness.get_start_event(), illness.get_end_event(), "Illness")
    ...
    def delete(self, illness):
        ...
        command = "DELETE FROM worktimeapp.illnesses WHERE id={}".format(
            illness.get_id())
        cursor.execute(command)
    ...
    def find_by_date(self, date):
        result = None
        cursor = self._cnx.cursor()
        command = "SELECT id, dateOfLastChange, start, end, startEvent, endEvent, type FROM worktimeapp.illnesses WHERE start={}".format(
            date)
        cursor.execute(command)
        tuples = cursor.fetchall()
    ...
    def find_by_time_period(self, start_date, end_date):
        result = []
        cursor = self._cnx.cursor()
        command = "SELECT id, dateOfLastChange, start, end, startEvent, endEvent, type FROM worktimeapp.illnesses WHERE start>={} AND end<={}".format(
            start_date, end_date)
        cursor.execute(command)
        tuples = cursor.fetchall()
