...
class IllnessMapper(TimeIntervalMapper):
    ...
    def find_all(self):
        ...
        cursor.execute("SELECT id, dateOfLastChange, start, end, timeIntervalId, startEvent, endEvent, type from worktimeapp.illnesses")
    ...
    def find_by_time_period(self, start_date, end_date):
        result = []
        cursor = self._cnx.cursor()
        command = "SELECT id, dateOfLastChange, start, end, timeIntervalBookingId, startEvent, endEvent, type FROM worktimeapp.illnesses WHERE start>={} AND end<={}".format(start_date, end_date)
        cursor.execute(command)
        tuples = cursor.fetchall()
